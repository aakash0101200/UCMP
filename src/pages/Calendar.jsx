...
import EventModal from './EventModal'; // Make sure this component exists
...
const Calendar = () => {
  ...
  return (
    // <div className=" inline-block float-right mr-4 mb-4 min-h-auto bg-gray-100  rounded-xl ">
        <div className=" inline-block float-right mr-4 mb-4 min-h-auto bg-gray-100  rounded-xl ">

      <div className="abssolute right-4 bottom-4 w-full sm:max-w-[700px] bg-white p-4 sm:p-6 rounded-xl shadow-md">
        {/* Header */}
            ...
            {days.map((day) => {
              ...
              return (
                <div
                  ...
                  className={`relative p-2 border-t border-r cursor-pointer transition-colors duration-200
                    ${!isSameMonth(day, monthStart) ? 'bg-gray-50 text-gray-400' : 'bg-white hover:bg-blue-50'}
                    ${isSelected ? 'bg-blue-100' : ''}
                    aspect-square overflow-hidden`}
                >
                  <div className="absolute top-1 left-1">
                    <span className={`text-sm font-medium ${isToday(day) ? 'bg-blue-600 text-white rounded-full h-7 w-7 flex items-center justify-center' : ''}`}>
                      {format(day, 'd')}
                    </span>
                  </div>
