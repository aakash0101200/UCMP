  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <Loader2 className={`h-8 w-8 animate-spin ${theme.loaderText}`} />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            {availableTerms.length > 0 ? (
              <select
                ...
                className={`text-xs border border-border/50 rounded px-1.5 py-0.5 bg-background text-foreground shadow-sm font-medium focus:ring-1 ${theme.focusRing} outline-none cursor-pointer`}
              >
                {availableTerms.map(t => (
                  <option key={t} value={t} className="bg-white dark:bg-[#14221C] text-slate-900 dark:text-slate-100">{t}</option>
                ))}
              </select>
            ) : (
        ...
        {entries.length > 0 && (
          <div className={`flex gap-1 ${theme.activeTabWrap} self-start sm:self-auto`}>
            {!isMobile && (
              <button
                onClick={() => setViewMode('grid')}
                className={`px-3 py-1.5 text-xs rounded-md transition-all flex items-center gap-1.5
                  ${viewMode === 'grid' ? theme.activeTabBtn : theme.inactiveTabBtn}`}
              >
            ...
            <button
              ...
              className={`px-3 py-1.5 text-xs rounded-md transition-all flex items-center gap-1.5
                ${viewMode === 'list' ? theme.activeTabBtn : theme.inactiveTabBtn}`}
            >
        ...
        <>
          ...
          {isMobile && viewMode === 'list' && (
            <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-hide">
              ...
              ${mobileDay === 'ALL' ? theme.activeDayBtn : (isFaculty ? 'bg-slate-100 text-slate-500 hover:bg-slate-200 dark:bg-[#14221C] dark:text-slate-400' : 'bg-muted text-muted-foreground hover:bg-muted/80')}`}
              >
                All
              </button>
              {DAYS.map(day => {
                ...
                return (
                  ...
                  >
                    ...
                    {day === today && mobileDay !== day && (
                      <span className={`absolute -top-0.5 -right-0.5 h-1.5 w-1.5 rounded-full ${theme.currentSlotDot}`} />
                    )}
                  </button>
                );
              })}
                ...
                <thead>
                  <tr>
                    ...
                    {DAYS.map((day) => (
                      ...
                      ${day === today ? theme.todayColHeader : 'text-muted-foreground'}`}
                      >
                        {DAY_SHORT[day]}
                    ...
                    return (
                      <tr key={time} className={isLunch ? 'bg-muted/30' : ''}>
                        ...
                        {DAYS.map((day) => {
                          ...
                          if (!entry) {
                            return (
                              <td key={day} className={`p-0.5 lg:p-1 border border-border/20 ${day === today ? theme.cellEmptyBg : ''}`}>
                          ...
                          return (
                            <td key={day} className="p-0.5 lg:p-1 border border-border/20">
                              ...
                              ${isCurrent ? theme.currentSlotRing : ''}`}
                              >
                                {isCurrent && !isCancelled && (
                                  <span className={`absolute top-1 right-1 h-2 w-2 rounded-full ${theme.currentSlotDot} animate-pulse`} />
                                )}
            ...
            <div className="space-y-4">
              {DAYS
                ...
                .map((day) => {
                  ...
                  return (
                    <div key={day}>
                      <h3 className={`text-sm font-semibold mb-2 ${day === today ? theme.primaryText : 'text-muted-foreground'}`}>
      ...
      {selectedEntryForCancel && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className={`w-full max-w-md rounded-2xl p-6 shadow-2xl animate-in fade-in zoom-in-95 text-left border ${theme.cancellationModal}`}>
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-2 text-rose-500">
                <AlertTriangle className="w-5 h-5" />
                <h3 className="text-lg font-bold">Cancel Class Exception</h3>
              </div>
              <button
            ...
            <form onSubmit={handleCancelSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Cancellation Date</label>
                <input
                  type="date"
                  required
                  value={cancellationDate}
                  onChange={e => setCancellationDate(e.target.value)}
                  className={`w-full px-3 py-2 text-sm rounded-xl focus:outline-none border ${theme.modalInput}`}
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Reason for Cancellation</label>
                <textarea
                  required
                  ...
                  rows={3}
                  className={`w-full px-3 py-2 text-sm rounded-xl focus:outline-none resize-none border ${theme.modalInput}`}
                />
              </div>

              <div className="p-3 bg-rose-500/10 border border-rose-500/20 rounded-xl text-[11px] text-rose-600 dark:text-rose-400 leading-relaxed">
              ...
              <div className="flex gap-3 justify-end pt-2">
                <button
                  type="button"
                  onClick={() => setSelectedEntryForCancel(null)}
                  className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${theme.cancelKeepBtn}`}
                >
                  Keep Scheduled
                </button>
                <button
